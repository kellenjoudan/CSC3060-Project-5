...
void stu_filter_gradient(float& out, const data_struct& data,
    ...
    // FULL ROW BUFFERS
    std::vector<float> ha(W * H), hb(W * H), hc(W * H);

    // PASS 1: horizontal sums
    for (size_t y = 0; y < H; ++y) {
        const float* a = &data.a[y * W];
        const float* b = &data.b[y * W];
        const float* c = &data.c[y * W];
        ...
        for (size_t x = 1; x + 1 < W; ++x) {
            ra[x] = a[x-1] + a[x] + a[x+1];
            rb[x] = b[x-1] + b[x] + b[x+1];
            rc[x] = c[x-1] + c[x] + c[x+1];
        }
    }

    // PASS 2: vertical + rest
    for (size_t y = 1; y + 1 < H; ++y) {

        const float* a0 = &ha[(y - 1) * W];
        const float* a1 = &ha[y * W];
        const float* a2 = &ha[(y + 1) * W];

        const float* b0 = &hb[(y - 1) * W];
        const float* b1 = &hb[y * W];
        const float* b2 = &hb[(y + 1) * W];

        const float* c0 = &hc[(y - 1) * W];
        const float* c1 = &hc[y * W];
        const float* c2 = &hc[(y + 1) * W];
        ...
        for (size_t x = 1; x + 1 < W; ++x) {
            ...
            // BOX FILTER
            float sum_a = a0[x] + a1[x] + a2[x];
            float sum_b = b0[x] + b1[x] + b2[x];
            float sum_c = c0[x] + c1[x] + c2[x];
            ...
            // SOBEL Y
            float dy_g =
                -g0[x-1] -2*g0[x] - g0[x+1] +
                 g2[x-1] +2*g2[x] + g2[x+1];

            float dy_h =
                -h0[x-1] -2*h0[x] - h0[x+1] +
                 h2[x-1] +2*h2[x] + h2[x+1];

            float dy_i =
                -i0[x-1] -2*i0[x] - i0[x+1] +
                 i2[x-1] +2*i2[x] + i2[x+1];
